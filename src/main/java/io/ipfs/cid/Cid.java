...
public class Cid extends Multihash {
  ...
  public enum Codec {
    ...
    public static Codec lookup(long c) {
      ...
      if (codec == null) throw new IllegalStateException("Unknown Codec type: " + c);
      return codec;
    }

    public static Codec lookupIPLDName(String name) {
      Codec codec = nameLookup.get(name);
      if (codec == null) throw new IllegalStateException("Unknown Codec type: " + name);
      return codec;
    }
  }
  ...
  @Override
  public byte[] toBytes() {
    if (version == 0) return toBytesV0();
    else if (version == 1) return toBytesV1();
  ...
  public static Cid decode(String v) {
    if (v.length() < 2) throw new IllegalStateException("CID too short: " + v);
  ...
  public static Cid cast(byte[] data) {
    if (data.length == 34 && data[0] == 18 && data[1] == 32)
      return buildV0(
          new Multihash(Type.lookup(data[0] & 0xff), Arrays.copyOfRange(data, 2, data.length)));
